...
The first move of a design is an importance ordering, not a layout. Decide what is most important, then what is second, then what is third. Assign visual weight down that ranking in decisive steps—each level unmistakably quieter than the one above—so nothing competes with the one thing that must win.

I used to shorthand this as 1-3-9. The numbers only ever described the decisiveness of the steps—one thing roughly nine times as visually weighty as the field it sits in—not quantities to count or values to compute.

## Judge with it, do not compute with it

Visual weight has no unit. Exact type sizes come from the type scale, spacing from the spacing ladder, and color from the scales; weight emerges from how those choices combine. Use the ranking to ask whether the rendered design points where it should, then adjust with the applicable system.
